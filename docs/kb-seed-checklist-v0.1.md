...
- `degraded_input_examples` 需要覆盖当前 8 个 baseline `failure_code`，并与 repair template、validator target、repair scope 保持一致，同时带出显式 negative-boundary marker，说明哪些层不能被顺手改写；其中 `continuity_break / handoff_gap / chinese_prompt_noise / export_contract_drift` 还需要至少各有 2 条样例，把 `runtime-consumer / exporter` 侧退化也纳入回归。
- `classic_case_examples` 需要同时满足无占位符脏文本、导演/委员会引用合法，以及当前最小分布 `director_classic=3 / benchmark_reference=3 / director_few_shot=5 / committee_handoff=6 / failure_repair=11`；其中 `continuity_break / handoff_gap / chinese_prompt_noise / export_contract_drift` 需要各有一条 support-facing `failure_repair`，覆盖 `exporter / runtime consumer / PromptPackage payload` 漂移。
